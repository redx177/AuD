...
public class Tree {
	...
	public Tree balance() {
		...
		setElementOrdered(elements);
		return balanceRunner(elements);
	}

	private Tree balanceRunner(List<Integer> elements) {
		int middle = (int) (Math.ceil(((double)elements.size())/2)-1);
		Tree tree = new Tree(elements.get(middle));

		if (middle != 0) {
			List<Integer> left = elements.subList(0, middle);
			tree.setLeft(balanceRunner(left));
		}

		if (elements.size() > 1) {
			List<Integer> right = elements.subList(middle+1, elements.size());
			tree.setRight(balanceRunner(right));
		}

		return tree;
	}

	private List<Integer> setElementOrdered(List<Integer> elements) {
		if (getLeft() != null) {
			getLeft().setElementOrdered(elements);
		}

		elements.add(getData());

		if (getRight() != null) {
			getRight().setElementOrdered(elements);
		}
